param(
    [string]$InputCsv = (Join-Path $PSScriptRoot '..\reports\lgbm_csi2000_hedge_v1_curves.csv'),
    [string]$OutputPng = (Join-Path $PSScriptRoot '..\reports\lgbm_csi2000_hedged_vs_indices_2020_now.png')
)
...
if ($rows.Count -lt 2) { throw 'The comparison CSV does not contain enough observations.' }

$series = @(
    [pscustomobject]@{ Key = 'current_best_lgbm'; Label = 'Base 75/25 LGBM'; Color = [System.Drawing.Color]::FromArgb(23, 107, 135) },
    [pscustomobject]@{ Key = 'lgbm_csi2000_hedged'; Label = 'LGBM + CSI 2000 hedge'; Color = [System.Drawing.Color]::FromArgb(33, 128, 91) },
...
$values = @{}
foreach ($item in $series) { $values[$item.Key] = New-Object 'System.Collections.Generic.List[double]' }
foreach ($row in $rows) {
    $dates.Add([datetime]::ParseExact($row.date, 'yyyy-MM-dd', $culture))
    foreach ($item in $series) { $values[$item.Key].Add([double]::Parse($row.($item.Key), $culture)) }
}
...
function Get-Y([double]$Value) { [single]($top + $plotHeight * ($yMaximum - $Value) / ($yMaximum - $yMinimum)) }
function Draw-SeriesSegment {
    param([System.Drawing.Graphics]$Graphics, [System.Drawing.Pen]$Pen, [System.Collections.Generic.List[double]]$SeriesValues, [int]$StartIndex, [int]$EndIndex)
...
$titleFont = [System.Drawing.Font]::new('Segoe UI', 23, [System.Drawing.FontStyle]::Regular)
...
try {
    $graphics.DrawString('LGBM with CSI 2000 hedge vs A-share indices: 2020 to latest', $titleFont, $textBrush, $left, 28)
    $graphics.DrawString('Start NAV = 1.00 | Hedge proxy: below MA120, short 75%, 1-session lag, 2 bp per change', $smallFont, $mutedBrush, $left, 74)
    $graphics.DrawString('CSI 2000 has no standard index futures; short-return curve is research-only. ETF-calibrated proxy after 2025-12-31.', $smallFont, $mutedBrush, $left, 96)
    ...
    for ($year = $dates[0].Year; $year -le $dates[$dates.Count - 1].Year; $year++) {
        ...
        $size = $graphics.MeasureString([string]$year, $smallFont)
        $graphics.DrawString([string]$year, $smallFont, $mutedBrush, $tickX - $size.Width / 2, $top + $plotHeight + 15)
    }
    ...
    foreach ($item in $series) {
        ...
        try {
            if ($item.Key -eq 'csi2000') {
                ...
                try { Draw-SeriesSegment $graphics $proxyPen $values[$item.Key] ([math]::Max(0, $proxyIndex - 1)) ($dates.Count - 1) }
                finally { $proxyPen.Dispose() }
            } else {
                Draw-SeriesSegment $graphics $solidPen $values[$item.Key] 0 ($dates.Count - 1)
            }
        } finally { $solidPen.Dispose() }
    ...
    $graphics.DrawString($axisLabel, $bodyFont, $mutedBrush, $left + ($plotWidth - $axisSize.Width) / 2, $height - 48)
    $bitmap.Save($OutputPng, [System.Drawing.Imaging.ImageFormat]::Png)
}
finally {
    $framePen.Dispose(); $gridPen.Dispose(); $mutedBrush.Dispose(); $textBrush.Dispose()
    $smallFont.Dispose(); $bodyFont.Dispose(); $titleFont.Dispose(); $graphics.Dispose(); $bitmap.Dispose()
}
